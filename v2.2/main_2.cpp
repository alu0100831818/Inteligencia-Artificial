...
int main(void){
    ...
    int selec, porciento,num_obstaculos=0, k=0,num_pas=0;
    int pasagger[num_pas][2];
    //limpiar pantalla
   ...
   //-----------------------------------------------
    ...
    if(select2==2){
        ...
        for (int i=0;i<num_pas;i++){
            cout<<"Introduzca las posiciones del pasajero Nº"<<i+1<<endl;
            for (int j=0;j<2;j++){
                cout<<"     Posicion ";cout<<j+1;cout<<":";
                cin>>pasagger[i][j];
    ...
    //-----------------------------------------------
        ...
        if(select2==2){
            obstaculos_.pasajeros(pasagger, num_pas);
        }
        else{
